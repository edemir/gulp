...
#define WRITE_PRIO	0	/* niceness value for Writer thread */
#define READER_CPU	2	/* assign Reader thread to this CPU */
#define WRITER_CPU	4	/* assign Writer thread to this CPU */
...
long long captured_len = 0;
long long captured_len_s = 0;
...
newoutfile(char *dir, int num) {
    ...
    snprintf(ofile, sizeof(ofile), "%s/pcap_%09d", dir, num);
...
int main(int argc, char *argv[], char *envp[])
    ...
    while (!eof) {
        struct timespec sn;
	usleep(500000);
	clock_gettime(CLOCK_MONOTONIC_RAW, &sn);
	double elapsed = (sn.tv_sec - st.tv_sec) + (double) (sn.tv_nsec - st.tv_nsec) / 1000000000;
		fprintf(stderr, "%f\n", elapsed);
        if (elapsed > 60) {
		long long d;
	  clock_gettime(CLOCK_MONOTONIC_RAW, &st);
	  d = captured_len - captured_len_s;
	  captured_len_s = captured_len;
	  d_rate = d / elapsed;
        }
	push += 1;
	/*
	...
	if (ps_stat_ptr) {
		...
		for (drop_symb = 0; drop_symb < 9; ++drop_symb) {
	       ...
	       if (pcs.ps_recv > 0) drop_rate = (double) pcs.ps_drop / pcs.ps_recv;
		}
#endif /* JUSTCOPY */
...
*/
		sprintf(sbuf, "%.0lf,%.0lf,%.0lf%%",
		    100.0 * drop_rate,	// a digit from 0-9 
		    100.0*(double)used/(double)(ringsize),
		    100.0*(double)maxbuffered/(double)(ringsize));
		...
		fprintf(stderr,
		    "pkts dropped: %d (%d) %1lf%%, ring buf: %.1lf%%, max: %.1lf%% rate: %f\n",
